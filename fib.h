...
struct fibStruct {
    ...
    unsigned long long int* fib;				    // The array which holds the calculated values
};
...
unsigned long long int fibValue(struct fibStruct*, size_t index);   // Find the index-th value in the populated fibonacci sequence. Returns NULL if you go our of bounds.
#endif /* _THE2B_FIB_H */
